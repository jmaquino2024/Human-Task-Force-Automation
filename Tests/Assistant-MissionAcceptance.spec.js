...
await page.locator('div:nth-child(3) > .table-auto > tbody > .rounded-xl > td:nth-child(6)').click();
...
// Wait for the page to reload after accepting the task
await page.waitForLoadState('load');

// Locate the task title
const taskTitleLocator = await page.getByText('Secrets of the Silent Sea');

// Verify if the "Review" status is present next to the task title
const reviewStatusLocator = await taskTitleLocator.locator('..').locator('text=Review');

if (await reviewStatusLocator.count() > 0) {
    console.log('The task mission status is "Review".');
} else {
    console.log('The task mission status is not "Review".');
}
