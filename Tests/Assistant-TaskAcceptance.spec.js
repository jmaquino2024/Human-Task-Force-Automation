...
const comments = [
    ...
    'Test message generated by automation'
  ];
  
  // Select a random comment from the array
  const randomComment = comments[Math.floor(Math.random() * comments.length)];
  
  // Fill and send the random comment
  await page.locator('div').filter({ hasText: /^Send$/ }).locator('div').nth(1).click();
  await page.locator('div').filter({ hasText: /^Send$/ }).locator('div').nth(1).fill(randomComment);
  await page.getByRole('button', { name: 'Send' }).click();
  
  // Log the random comment that was added
  console.log(`Filled and sent a comment: "${randomComment}".`);
